...
public class SolarAzimuth {
  ...
  public SolarAzimuth(double latitude, double longitude, boolean dst) {
    this.latitude = latitude;
    this.longitude = longitude;
    this.dst = dst;
  }

  public SolarAzimuth(double latitude, double longitude) {
    this.latitude = latitude;
    this.longitude = longitude;
    this.dst = true;
  }
  ...
  private double calcSunEqOfCenter(double timeJulian) {
    ...
    double output = TrigCalc.sinD(m)*(1.914602 - timeJulian*(0.004817 + 0.000014*timeJulian)) + TrigCalc.sinD(m*2)*(0.019993 - 0.000101*timeJulian) + TrigCalc.sinD(m*3)*0.000289;
    return output;
  }
  ...
  private double calcSunApparentLong(double timeJulian) {
    ...
    double output = o - 0.00569 - 0.00478 * TrigCalc.sinD(omega);
    return output;
  }
  ...
  private double calcObliquityCorrection(double timeJulian) {
    ...
    double omega = 125.04 - 1934.136*timeJulian;
    double output = e0 + 0.00256*TrigCalc.cosD(omega);
    return output;
  }
  ...
  public double calcSolarDeclination(Calendar cal) {
    ...
    double sint = TrigCalc.sinD(e)*TrigCalc.sinD(lambda);
  ...
  public double calcEquationOfTime(Calendar cal) {
    ...
    double y = TrigCalc.tanD(epsilon/2);
    y *= y;
    double sin2l0 = TrigCalc.sinD(2*l0);
    double sinm = TrigCalc.sinD(m);
    double cos2l0 = TrigCalc.cosD(2*l0);
    double sin4l0 = TrigCalc.sinD(4*l0);
    double sin2m = TrigCalc.sinD(2*m);
  ...
  public double calcSolarZenith(Calendar cal, boolean refraction) {
    ...
    double csz = TrigCalc.sinD(this.latitude)*TrigCalc.sinD(solarDeclination) +
            TrigCalc.cosD(this.latitude)*TrigCalc.cosD(solarDeclination)*TrigCalc.cosD(hourAngle);
    double solarZenith = Math.toDegrees(Math.acos(csz));

    if (refraction) {
      double solarElevation = 90 - solarZenith;
      double refractionCorrection = 0;
      double te = TrigCalc.tanD(solarElevation);
  ...
  public double calcSolarAzimuth(Calendar cal) {
    ...
    double output = Math.toDegrees( Math.signum(hourAngle) * Math.acos( (TrigCalc.cosD(solarZenith)*TrigCalc.sinD(this.latitude) - TrigCalc.sinD(solarDeclination)) / (TrigCalc.sinD(solarZenith)*TrigCalc.cosD(this.latitude)) ) );
    return output;
  }
  ...
  public double calcAirMass(Calendar cal) {
    ...
    double output = 0;
    if (solarZenith < 90) output = (1.002432*Math.pow(TrigCalc.cosD(solarZenith), 2) + 0.148386*TrigCalc.cosD(solarZenith) + 0.0096467) / (Math.pow(TrigCalc.cosD(solarZenith), 3) + 0.149864*Math.pow(TrigCalc.cosD(solarZenith), 2) + 0.0102963 * TrigCalc.cosD(solarZenith) + 0.000303978);
    return output;
  }
  ...
  public double calcExtraIrradiance(Calendar cal) {
    ...
    double output = I_SC*(1.00011 + 0.034221*TrigCalc.cosD(360*doy/365) + 0.00128*TrigCalc.sinD(360*doy/365) + 0.000719*TrigCalc.cosD(2*360*doy/365) + 0.000077*TrigCalc.sinD(2*360*doy/365));
    return output;
  }
  ...
  private double calcHourAngleSunrise(Calendar cal) {
    double solarDeclination = calcSolarDeclination(cal);
    double hourAngle = Math.toDegrees(Math.acos(TrigCalc.cosD(90.833)/(TrigCalc.cosD(this.latitude)*TrigCalc.cosD(solarDeclination))-TrigCalc.tanD(this.latitude) * TrigCalc.tanD(solarDeclination)));
    return hourAngle;
  }
